...
Isaac_Tower.FlayerHandlers.DeadZone = .1
...
function Inp.PressDown(idx)
	if idx == 0 then
		return Input.GetActionValue(ButtonAction.ACTION_SHOOTDOWN, idx)  --IsActionPressed
	else
		return Input.GetActionValue(ButtonAction.ACTION_DOWN, idx)
	end
end
function Inp.PressUp(idx)
	if idx == 0 then
		return Input.GetActionValue(ButtonAction.ACTION_SHOOTUP, idx)
	else
		return Input.GetActionValue(ButtonAction.ACTION_UP, idx)
	end
end
...
function Inp.PressRunOnce(idx)
	if idx == 0 then
		return Input.IsButtonTriggered(Keyboard.KEY_LEFT_SHIFT, idx)
	else
...
function Inp.PressJumpOnce(idx, fent)
	local ret
	if idx == 0 then
		ret = Input.IsButtonTriggered(Keyboard.KEY_V, idx)
	else
...
function Inp.PressGrabOnce(idx)
	if idx == 0 then
		return Input.IsButtonTriggered(Keyboard.KEY_S, idx)
	else
		return Input.IsActionTriggered(ButtonAction.ACTION_SHOOTLEFT, idx)
	end
end
...
function Isaac_Tower.FlayerHandlers.GrabHandler(fent, spr)
	...
	if Inp.PressGrab(idx) and fent.GrabDelay <= 0 and not fent.GrabPressed then
		...
		if fent.State == "Бег_по_стене" then
			...
			if Inp.PressUp(idx) > handler.DeadZone and not fent.UseApperkot then
...
Isaac_Tower.FlayerMovementState["Ходьба"] = function(player, fent, spr, idx)
	if player.ControlsEnabled then
		...
		--		return
		--	end
		elseif Inp.PressDown(idx) > handler.DeadZone then
...
Isaac_Tower.FlayerMovementState["НачалоБега"] = function(player, fent, spr, idx)
	...
	if player.ControlsEnabled then
		...
		if fent.CollideWall and not (Inp.PressDown(idx) > handler.DeadZone) then --and fent.CollideWall == sign0(rot) then
		...
		if Inp.PressDown(idx) > handler.DeadZone then
			if fent.OnGround then
...
Isaac_Tower.FlayerMovementState["Бег"] = function(player, fent, spr, idx)
	if player.ControlsEnabled then
		local rot = -Inp.PressLeft(idx) + Inp.PressRight(idx)
		local press = Inp.PressLeft(idx) + Inp.PressRight(idx) ~= 0
		local onground = fent.OnGround

		if fent.slopeAngle and onground then
		...
		if not (Inp.PressDown(idx) > handler.DeadZone) then
			...
			if fent.CollideWall and fent.slopeAngle and Isaac_Tower.FlayerHandlers.IsCanWallClamb(fent, rot) then
				...
				fent.JumpActive = 30
				return
			elseif fent.CollideWall and not onground and Isaac_Tower.FlayerHandlers.IsCanWallClamb(fent, rot) then
				spr.Rotation = 0
				spr.Offset = fent.Flayer.DefaultOffset
				...
				spr:Play("wall_climbing", true)
				return
			elseif fent.CollideWall and onground then --fent.Velocity.X == 0 then
		...
		if rot and press then
			...
			if press and sign0(rot) ~= sign0(fent.RunSpeed) and math.abs(fent.RunSpeed) > 6.4 and onground then
		...
		if onground then
			...
			if Inp.PressUp(idx) > handler.DeadZone then
		...
		local ignorelist = {["lunge_down"]=true,["run_jump"]=true}
		if onground or not ignorelist[spr:GetAnimation()] then
		...
		--if onground and math.abs(fent.RunSpeed) < handler.RunSpeed3 and Inp.PressJumpOnce(idx, fent) then
		...
		if fent.IsLunge then
			fent.CanJump = false
			fent.Half = fent.DefaultCroachHalf/1
		...
		if spr:IsFinished("run_jump") then
			spr:Play("run")
		end
		if Isaac_Tower.FlayerHandlers.GrabHandler(fent, spr) then
			return
...
Isaac_Tower.FlayerMovementState["Присел"] = function(player, fent, spr, idx)
	if player.ControlsEnabled then
		...
		if not (Inp.PressDown(idx) > handler.DeadZone) and CheckCanUp(player) then
			SetState(fent, "Ходьба")--fent.State = 1
			fent.Half = fent.DefaultHalf/1 --Vector(15,20)
			fent.CollisionOffset = Vector(0,0)
		end
...
Isaac_Tower.FlayerMovementState["Скольжение_Захват"] = function(player, fent, spr, idx)
	if player.ControlsEnabled then
		...
		if fent.SlideTime<=0 and not (Inp.PressDown(idx) > handler.DeadZone) and CheckCanUp(player) then
			SetState(fent, "Бег")--fent.State = 3
			fent.Half = fent.DefaultHalf/1 --Vector(15,20)
			fent.CollisionOffset = Vector(0,0)
			fent.IsLunge = true
			...
			Isaac_Tower.HandleMoving(player) --player:Update()
			return
		elseif fent.SlideTime <= 0 and not (Inp.PressDown(idx) > handler.DeadZone) then
...
Isaac_Tower.FlayerMovementState["Захват"] = function(player, fent, spr, idx)
	...
	if player.ControlsEnabled then
		...
		if fent.StateFrame > 1 and not (Inp.PressDown(idx) > handler.DeadZone) --and Inp.PressRun(idx) 
		and fent.CollideWall and Isaac_Tower.FlayerHandlers.IsCanWallClamb(fent, rot) then
			...
			--if Isaac_Tower.MovementHandlers.IsCanWallClamb(fent, rot) then
				...
				return
			--end
		elseif fent.StateFrame > 1 and not (Inp.PressDown(idx) > handler.DeadZone) and fent.CollideWall then
		...
		if Inp.PressDown(idx) > handler.DeadZone then
			if fent.OnGround then ---and fent.StateFrame < 15 then
...
Isaac_Tower.FlayerMovementState["Захватил"] = function(player, fent, spr, idx)
	...
	if fent.StateFrame == 1 then
		...
		fent.ExtraSpeed = fent.RunSpeed/1
		CleanOnStateChange(fent, "ExtraSpeed")
	end
	...
	if player.ControlsEnabled then --and not spr:IsPlaying("holding_appear") then
		...
		local rot = -Inp.PressLeft(idx) + Inp.PressRight(idx)
		if rot<0 then --Inp.PressLeft(idx)>0 then
			fent.RunSpeed = Inp.PressLeft(idx)*-4 + (fent.ExtraSpeed or 0)
			rot = -4
		elseif rot>0 then --Inp.PressRight(idx)>0 then
			fent.RunSpeed = Inp.PressRight(idx)*4 + (fent.ExtraSpeed or 0)
			rot = 4
		else
			fent.RunSpeed = 0
		end
		
		if fent.ExtraSpeed then
			if rot<0 then
				fent.RunSpeed = math.max(fent.RunSpeed, math.min(-4, fent.ExtraSpeed))
			else
				fent.RunSpeed = math.min(fent.RunSpeed, math.max(4, fent.ExtraSpeed))
			end
			fent.ExtraSpeed = fent.ExtraSpeed * 0.8
			if fent.ExtraSpeed < .1 then
				fent.ExtraSpeed = nil
			end
		end
...
Isaac_Tower.FlayerMovementState["Захватил ударил"] = function(player, fent, spr, idx)
	...
	elseif not spr:WasEventTriggered("hit") then
		fent.Velocity = fent.Velocity * 0.85
		fent.RunSpeed = fent.RunSpeed * 0.85
...
Isaac_Tower.FlayerMovementState["Супер_прыжок_подготовка"] = function(player, fent, spr, idx)
	...
	if player.ControlsEnabled and Flayer.Queue == -1 then
		...
		if Inp.PressUp(idx) > handler.DeadZone and fent.OnGround then
			local rot = -Inp.PressLeft(idx) + Inp.PressRight(idx)
			if rot<0 then
...
Isaac_Tower.FlayerMovementState["Супер_прыжок_перенаправление"] = function(player, fent, spr, idx)
	...
	if spr:IsFinished(spr:GetAnimation()) then
		if fent.OnGround then
			...
			if not fent.OnGround and Inp.PressDown(idx) > handler.DeadZone then
				SetState(fent, "Бег")--fent.State = 3
				fent.Half = fent.DefaultHalf/1 --Vector(15,20)
				fent.CollisionOffset = Vector(0,0)
				fent.IsLunge = true
			...
			if not (Inp.PressDown(idx) > handler.DeadZone) and fent.CollideWall == sign0(fent.RunSpeed) then
